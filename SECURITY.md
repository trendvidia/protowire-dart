...
This policy covers `protowire-dart` — the Dart port of the `protowire`
...
- Schema-validation bypasses that let invalid messages reach
  application code.
- Soundness issues in any path that uses `dart:ffi` (currently none —
  this port is pure Dart) or `dart:typed_data` reinterpretation.
...
- Issues in `package:protobuf` itself — file those upstream at
...
on every PR via the `bin/check_decode.dart` harness. A regression on
the corpus blocks merge.

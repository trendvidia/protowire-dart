...
void _collectTypes(BuilderInfo info, Set<int> strLengths, List<BuilderInfo> composites, Set<String> compSeen, List<_EnumInfo> enums, Set<String> enumSeen) {
  for (final tag in info.fieldInfo.keys) {
    ...
    if ((fi.type & _ENUM_BIT) != 0) {
      final name = fi.name; // Use field name as hint for enum name if not available
    ...
    if ((fi.type & _MESSAGE_BIT) != 0) {
      ...
      if (!compSeen.contains(name)) {
        ...
        _collectTypes(subInfo, strLengths, composites, compSeen, enums, enumSeen);
      }
    }
  }
}
...
void _writeXmlEnum(StringBuffer sb, _EnumInfo ei) {
  ...
  for (final v in ei.values) {
    sb.writeln('            <validValue name="${v.name}">${v.value}</validValue>');
  }
  sb.writeln('        </enum>');
}
...
void _writeXmlComposite(StringBuffer sb, BuilderInfo info) {
  ...
  for (final tag in sortedTags) {
    ...
    if (sbeType.length > 0) {
        sb.writeln('            <type name="$fieldName" primitiveType="${sbeType.primitiveType}" length="${sbeType.length}"/>');
    } else {
        sb.writeln('            <type name="$fieldName" primitiveType="${sbeType.primitiveType}"/>');
    }
  }
  sb.writeln('        </composite>');
}
...
void _writeXmlField(StringBuffer sb, FieldInfo fi, String indent) {
    final fieldName = snakeToCamel(fi.name);
    final type = (fi.type & _ENUM_BIT) != 0 ? fi.name : 
                 (fi.type & _MESSAGE_BIT) != 0 ? fi.subBuilder!().info_.messageName :
                 _protoFieldToSBEType(fi).xmlType;
    sb.writeln('$indent<field name="$fieldName" id="${fi.tagNumber}" type="$type"/>');
}
...
_SbeTypeInfo _protoFieldToSBEType(FieldInfo fi) {
    ...
    if ((type & _INT32_BIT) != 0 || (type & _SINT32_BIT) != 0 || (type & _SFIXED32_BIT) != 0) return _SbeTypeInfo('int32', 'int32', 0);
    if ((type & _INT64_BIT) != 0 || (type & _SINT64_BIT) != 0 || (type & _SFIXED64_BIT) != 0) return _SbeTypeInfo('int64', 'int64', 0);
    if ((type & _UINT32_BIT) != 0 || (type & _FIXED32_BIT) != 0) return _SbeTypeInfo('uint32', 'uint32', 0);
    if ((type & _UINT64_BIT) != 0 || (type & _FIXED64_BIT) != 0) return _SbeTypeInfo('uint64', 'uint64', 0);

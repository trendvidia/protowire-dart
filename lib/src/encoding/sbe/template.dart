...
MessageTemplate buildTemplate(
  ...
  for (final tag in sortedTags) {
    ...
    if (fi.isRepeated && (fi.type & _MESSAGE_BIT) != 0) {
      final gt =
          _buildGroupTemplate(fi, lengths: lengths, encodings: encodings);
      groups.add(gt);
      continue;
    }
    ...
    if (fi.isRepeated) {
      throw Exception(
          'sbe: repeated scalar field ${info.qualifiedMessageName}.${fi.name} not supported');
    }

    if ((fi.type & _MESSAGE_BIT) != 0) {
      final subInfo = fi.subBuilder!().info_;
      final composite = _buildCompositeFields(subInfo,
...
GroupTemplate _buildGroupTemplate(FieldInfo fi,
  ...
  for (final tag in sortedTags) {
    ...
    if ((sfi.type & _MESSAGE_BIT) != 0) {
      final composite = _buildCompositeFields(sfi.subBuilder!().info_,
          lengths: lengths, encodings: encodings);
      int size = 0;
      for (final f in composite) {
        size += f.size;
      ...
      fields.add(FieldTemplate(
          fi: sfi, offset: offset, size: size, composite: composite));
      offset += size;
      continue;
    }
    ...
    fields
        .add(FieldTemplate(fi: sfi, offset: offset, size: size, encoding: enc));
    offset += size;
  }
...
List<FieldTemplate> _buildCompositeFields(BuilderInfo info,
    {Map<int, int>? lengths, Map<int, String>? encodings}) {
  final sortedTags = info.fieldInfo.keys.toList()..sort();
  final fields = <FieldTemplate>[];
  int offset = 0;
  ...
  for (final tag in sortedTags) {
    final fi = info.fieldInfo[tag]!;
    if ((fi.type & _MESSAGE_BIT) != 0) {
      final composite = _buildCompositeFields(fi.subBuilder!().info_,
          lengths: lengths, encodings: encodings);
      int size = 0;
      for (final f in composite) {
        size += f.size;
      ...
      fields.add(FieldTemplate(
          fi: fi, offset: offset, size: size, composite: composite));
      offset += size;
      continue;
    }
    ...
    fields
        .add(FieldTemplate(fi: fi, offset: offset, size: size, encoding: enc));
    offset += size;
  }
  return fields;
}

(String, int) _fieldEncodingSize(
    FieldInfo fi, int? lengthOverride, String? encodingOverride) {
  ...
  if ((type & _STRING_BIT) != 0 || (type & _BYTES_BIT) != 0) {
    if (lengthOverride == null) {
      throw Exception(
          'sbe: string/bytes field ${fi.name} requires length override/annotation');
    }

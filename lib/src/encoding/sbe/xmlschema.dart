...
class XMLSchema {
  ...
  factory XMLSchema.fromXml(XmlDocument document) {
    ...
    final messages = root.findElements('message').map((e) => XMLMessage.fromXml(e)).toList();
...
class XMLComposite {
  ...
  factory XMLComposite.fromXml(XmlElement element) {
    final types = element.findElements('type').map((e) => XMLType.fromXml(e)).toList();
    final refs = element.findElements('ref').map((e) => XMLRef.fromXml(e)).toList();
    return XMLComposite(
      name: element.getAttribute('name') ?? '',
      description: element.getAttribute('description'),
      types: types,
...
class XMLEnum {
  ...
  factory XMLEnum.fromXml(XmlElement element) {
    final validValues = element.findElements('validValue').map((e) => XMLValidValue.fromXml(e)).toList();
    return XMLEnum(
      name: element.getAttribute('name') ?? '',
...
class XMLMessage {
  ...
  factory XMLMessage.fromXml(XmlElement element) {
    final fields = element.findElements('field').map((e) => XMLField.fromXml(e)).toList();
    final groups = element.findElements('group').map((e) => XMLGroup.fromXml(e)).toList();
    return XMLMessage(
      name: element.getAttribute('name') ?? '',

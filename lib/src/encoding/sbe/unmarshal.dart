...
void unmarshalMessage(
    Uint8List data, GeneratedMessage msg, MessageTemplate tmpl) {
  ...
  if (templateID != tmpl.templateID) {
    throw Exception(
        'sbe: template ID mismatch: got $templateID, want ${tmpl.templateID}');
  }
...
void _readField(
    ByteData buffer, int baseOffset, FieldTemplate ft, GeneratedMessage msg) {
...
void _setUintField(GeneratedMessage msg, FieldInfo fi, int v) {
  if ((fi.type & _BOOL_BIT) != 0) {
    ...
    msg.setField(fi.tagNumber, v);
  } else if ((fi.type & _UINT64_BIT) != 0 || (fi.type & _FIXED64_BIT) != 0) {
    msg.setField(fi.tagNumber, Int64(v));
  } else {
    msg.setField(fi.tagNumber, v);
  }

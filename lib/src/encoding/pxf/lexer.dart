...
class Lexer {
    ...
    while (pos < input.length) {
      ...
      if (ch == 0x22) {
        ...
        try {
          ...
          return Token(
              TokenKind.illegal, 'string is not valid UTF-8: ${e.message}', p);
        }
      }
      if (ch != 0x5C) {
  ...
  static int? _octVal(int ch) => (ch >= 0x30 && ch <= 0x37) ? ch - 0x30 : null;
  ...
  Token _lexDuration(Position p, int start) {
    while (pos < input.length &&
        (_isDigit(_peekCode) || _isLowerAlpha(_peekCode))) {
      _advance();
    }
    String raw = input.substring(start, pos);
  ...
  bool _isValidDuration(String s) {
    ...
    final durationRegex =
        RegExp(r'^([-+]?([0-9]+(\.[0-9]+)?(ns|us|µs|ms|s|m|h))+)$');
  ...
  bool _isIdentStart(int ch) =>
      ...
      (ch >= 65 && ch <= 90) ||
      ch == 95; // a-z, A-Z, _
  bool _isIdentPart(int ch) =>
      _isIdentStart(ch) || _isDigit(ch) || ch == 46; // ., digits

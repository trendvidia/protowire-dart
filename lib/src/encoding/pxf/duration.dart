...
Duration parseGoDuration(String s) {
  ...
  for (var match in matches) {
    ...
    if (part.endsWith('ns')) {
      ...
      totalMicroseconds +=
          double.parse(part.substring(0, part.length - 2)).round();
    } else if (part.endsWith('µs')) {
      totalMicroseconds +=
          double.parse(part.substring(0, part.length - 2)).round();
    } else if (part.endsWith('ms')) {
      totalMicroseconds +=
          (double.parse(part.substring(0, part.length - 2)) * 1000).round();
    } else if (part.endsWith('s')) {
      totalMicroseconds +=
          (double.parse(part.substring(0, part.length - 1)) * 1000000).round();
    } else if (part.endsWith('m')) {
      totalMicroseconds +=
          (double.parse(part.substring(0, part.length - 1)) * 60000000).round();
    } else if (part.endsWith('h')) {
      totalMicroseconds +=
          (double.parse(part.substring(0, part.length - 1)) * 3600000000)
              .round();
    }
  }

  return Duration(
      microseconds: negative ? -totalMicroseconds : totalMicroseconds);
}

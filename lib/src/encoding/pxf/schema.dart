...
//   `permissions` — future-reserved; v1 decoders MUST reject them so
//   applications cannot squat the names before the spec allocates
//   semantics.

const _futureReservedDirectives = <String>{
  'table',
  'datasource',
  'view',
  'procedure',
...
/// True when [name] is reserved for future allocation by draft §3.4.6
/// and MUST be rejected by v1 decoders. Names with their own lexer
/// production (`type`, `dataset`, `proto`) and the registered `entry`
/// are not included here — they're handled either at the lexer or by
/// the named-directive shape.
bool isFutureReservedDirective(String name) =>
    _futureReservedDirectives.contains(name);
